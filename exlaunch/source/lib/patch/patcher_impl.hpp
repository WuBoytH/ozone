...
namespace exl::patch {
    ...
    namespace impl {
        // Storage T is non-const; GCC 15's C++20 strict mode rejects std::construct_at<const T>,
        // and -fpermissive papers over it but leaves the compiler free to assume the storage is
        // immutable and optimize the post-init reads against uninitialized bytes. Dropping const
        // is the actual fix.
