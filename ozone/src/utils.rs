...
pub mod env {
    ...
    static PLATFORM: LazyLock<RunEnvironment> = LazyLock::new(|| {
        ...
        if dbg!(base_addr) == 0x8004000 || base_addr == 0x8504000 {
    ...
    pub fn get_game_version() -> Version {
        unsafe {
            ...
            Version::from_str(&skyline::from_c_str(version_string.name.as_ptr())).expect("Game version should parse as a proper semver.")
        }
    }
}
